...
def crawling_data_from_google():
    ...
    crawling_google_on_issued_date(soup_grant)
    logging.info('Start to crawl google application till 2015')
    crawling_google_on_issued_date(soup_application)


def crawling_google_on_issued_date(soup_page):
    for list in soup_page.findAll('h3'):
        if list['id'] <= '2015':
...
def crawling_data_from_reedtech():
    ...
    crawling_reedtech_on_issued_date(soup_grant)

    logging.info('start to crawl Reedtech application from 2016')
    crawling_reedtech_on_issued_date(soup_application)


def crawling_reedtech_on_issued_date(soup_page):
    for list in soup_page.findAll('div', {'class', 'bulkyear'}):
        if list['id'] > '2015':
